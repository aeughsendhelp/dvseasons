...
class VegetationPrefixPatch {
	static void Prefix(ref VegetationCellSpawner __instance) {
		...
		foreach(VegetationPackageProModelInfo item in modelList) {
			var newList = item.VegetationItemModelList;

			foreach(VegetationItemModelInfo veg in newList) {
				// Vegetation editing will go right here
				veg.VegetationMeshLod0 = new Mesh();
			}
		}

		__instance.VegetationPackageProModelsList = modelList;
	}
}
